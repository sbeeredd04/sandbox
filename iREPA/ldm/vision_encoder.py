...
# Import DGM models
from models.dgm import ResNet18, ResNet34
...
class DGMEncoder(VisionEncoder):
    """
    Deep Geometric Moments (DGM) encoder.
    
    DGM computes geometric moments spatially and outputs:
      - grid: [B, 256, H, W] - Spatial geometric moment features
      - xy1: [B, 256] - Global flattened features
    
    We use the spatial 'grid' output as patch tokens since it provides
    spatial representation similar to ViT patch embeddings.
    
    Output format: [B, T, 256] where T = H*W (spatial positions)
    
    Architecture options:
      - resnet18: Lighter model
      - resnet34: Standard model (recommended)
    
    Model config:
      - 'default': Uses checkpoint from pretrained_models/res34_model_best.pth.tar
      - Custom path: Full path to checkpoint file
    """
    ...
    def load_model(self):
        ...
        if self.architecture not in arch_map:
            raise ValueError(f"Unknown DGM architecture: {self.architecture}. "
                           f"Available: {list(arch_map.keys())}")
        
        # Create model
        model_class = arch_map[self.architecture]
        self.model = model_class(device=self.device, num_classes=1000)
        
        # Load checkpoint
        if self.model_config == 'default':
            checkpoint_path = PRETRAINED_DIR / "res34_model_best.pth.tar"
        else:
            checkpoint_path = Path(self.model_config)
        
        if not checkpoint_path.exists():
            raise FileNotFoundError(f"DGM checkpoint not found: {checkpoint_path}")
        
        print(f"[DGMEncoder] Loading checkpoint from: {checkpoint_path}")
        ...
        # DGM uses df=256 as feature dimension (defined in dgm.py MyResNet1.__init__)
        self._embed_dim = 256
        
        print(f"[DGMEncoder] Loaded {self.architecture} with embed_dim={self._embed_dim}")
        
    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        """
        DGM expects images in [0, 1] range.
        No ImageNet normalization needed - DGM has its own internal normalization.
        """
        # Convert from [0, 255] to [0, 1]
        x = x / 255.0
        # Resize if needed (DGM works with different resolutions)
    ...
    def forward_features(self, x: torch.Tensor) -> Dict[str, Optional[torch.Tensor]]:
        """
        Extract spatial features from DGM.
        
        DGM forward returns: (classification, (grid, xy1), attention_map)
          - grid: [B, 256, H, W] - Spatial geometric moment features
          - xy1: [B, 256] - Global pooled features
        
        We use 'grid' as it provides spatial information similar to ViT patch tokens.
        The grid is downsampled to 16x16 to match other encoders (256 tokens).
        
        Returns:
            dict with:
                - 'x_norm_clstoken': None (DGM doesn't have explicit CLS token)
                - 'x_norm_patchtokens': [B, 256, 256] (256 tokens with 256 dims)
        """
        # DGM forward returns: (classification, (grid, xy1), attention_map)
        _, (grid, xy1), _ = self.model(x, return_moments=True)
        
        # grid shape: [B, 256, H, W] - spatial geometric moment features (e.g., [B, 256, 32, 32])
        # Downsample to 16x16 to match other encoders (256 tokens like ViT)
        # This matches the SiT hidden representation size
        target_size = 16  # 16x16 = 256 tokens to match other encoders
        grid = torch.nn.functional.adaptive_avg_pool2d(grid, (target_size, target_size))
        
        # Convert to [B, T, D] format like ViT patch tokens
        B, C, H, W = grid.shape  # [B, 256, 16, 16]
        
        # Flatten spatial dimensions and transpose to [B, H*W, 256]
        # This matches the format expected by the loss function
        spatial_features = grid.flatten(2).transpose(1, 2)  # [B, 256, 256]
        
        # Return xy1 = [B, 256] as CLS token
        
        return {
            'x_norm_clstoken': xy1,  # Global pooled features as CLS token
            'x_norm_patchtokens': spatial_features
        }
...
ENCODER_REGISTRY = {
    ...
    # DGM encoder (Deep Geometric Moments)
    "dgm": DGMEncoder,
}
